...
case "$LANDMARK_MATCH_MODE" in
  image_obs|image_obs_hloc_nn)
    MEMORY_TAG="$LANDMARK_MATCH_MODE"
    ;;
  point_memory|point_memory_support|point_memory_hloc_nn|point_memory_imagewise_hloc_nn)
    case "$POINT_MEMORY_OBS_SELECT" in
      adaptive_cover|adaptive_cover_farthest|adaptive_cover_v2)
        MEMORY_TAG="${LANDMARK_MATCH_MODE}_${POINT_MEMORY_OBS_TAG}_k${POINT_MEMORY_ADAPTIVE_K_MIN}_${POINT_MEMORY_ADAPTIVE_K_MAX}_gain${GAIN_TAG}"
        ;;
      *)
        MEMORY_TAG="${LANDMARK_MATCH_MODE}_${POINT_MEMORY_OBS_TAG}_obs${POINT_MEMORY_MAX_OBS}"
        ;;
    esac
    ;;
  *)
    echo "Unsupported LANDMARK_MATCH_MODE=$LANDMARK_MATCH_MODE" >&2
    exit 2
    ;;
esac
RESULT_NAME=${RESULT_NAME:-plmloc_${FEATURE}_top${TOPK}_${MEMORY_TAG}_${PNP_TAG}}
...
ROBOT_SUB="$SUB/RobotCar_eval_PLMLoc_${FEATURE}_top${TOPK}_${MEMORY_TAG}_${PNP_TAG}_v2_test.txt"
