...
rm -rf "$OUT"
mkdir -p "$OUT"
if [[ "${#MICRO_ARGS[@]}" -eq 0 && "${#SEEDED_ARGS[@]}" -eq 0 && ( "$LOCAL_FEATURE_METHOD" == "superpoint_h5" || "$LOCAL_FEATURE_METHOD" == "superpoint-h5" || "$LOCAL_FEATURE_METHOD" == "sp_h5" || "$LOCAL_FEATURE_METHOD" == "sp-h5" ) ]]; then
